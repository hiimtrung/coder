...
func NewMemoryServer(mgr *memory.Manager) *MemoryServer {
...
func (s *MemoryServer) handleStore(w http.ResponseWriter, r *http.Request) {
	...
	id, err := s.mgr.Store(r.Context(), req.Title, req.Content, memory.MemoryType(req.Type), req.Metadata, req.Scope, req.Tags)
	if err != nil {
...
func (s *MemoryServer) handleSearch(w http.ResponseWriter, r *http.Request) {
	...
	var req struct {
		...
		Tags        []string               `json:"tags"`
		Type        string                 `json:"type"`
		MetaFilters map[string]interface{} `json:"meta_filters"`
	...
	results, err := s.mgr.Search(r.Context(), req.Query, req.Scope, req.Tags, memory.MemoryType(req.Type), req.MetaFilters, req.Limit)
	if err != nil {
		http.Error(w, err.Error(), http.StatusInternalServerError)
		return
	}
