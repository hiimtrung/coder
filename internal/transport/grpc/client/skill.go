...
func (c *skillClient) SearchSkills(ctx context.Context, query string, limit int) ([]skilldomain.SkillSearchResult, error) {
	...
	for _, r := range resp.Results {
		sr := skilldomain.SkillSearchResult{
		...
		for _, ch := range r.Chunks {
			sr.Chunks = append(sr.Chunks, skilldomain.SkillChunkResult{
				SkillChunk: skilldomain.SkillChunk{
					ID:         ch.Id,
					SkillID:    ch.SkillId,
...
func (c *skillClient) ListSkills(ctx context.Context, source, category string, limit, offset int) ([]skilldomain.Skill, error) {
	...
	for _, sk := range resp.Skills {
		skills = append(skills, skilldomain.Skill{
			ID:          sk.Id,
			Name:        sk.Name,
...
func (c *skillClient) GetSkill(ctx context.Context, name string) (*skilldomain.Skill, []skilldomain.SkillChunk, error) {
	...
	for _, ch := range resp.Chunks {
		chunks = append(chunks, skilldomain.SkillChunk{
			ID:         ch.Id,
			SkillID:    ch.SkillId,
			ChunkType:  ch.ChunkType,
			Title:      ch.Title,
...
func (c *skillClient) StoreSkillFiles(_ context.Context, _ string, _ []skilldomain.SkillFile) (int, error) {
	return 0, fmt.Errorf("file operations are not supported over gRPC; reconnect with --protocol http")
}

func (c *skillClient) GetSkillFiles(_ context.Context, _ string) ([]skilldomain.SkillFile, error) {
