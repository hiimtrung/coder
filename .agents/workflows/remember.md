...
1. **Gate In (MANDATORY)** — Run `coder memory search "<topic>"` to check if similar knowledge already exists before adding a new item.
...
4. **Gate Out (MANDATORY)** — Call `coder memory store` with title, content, tags, scope.
