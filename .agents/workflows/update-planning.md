...
1. **Gate In (MANDATORY)** — Run `coder memory search "<feature planning context>"` to retrieve previous milestones and delayed tasks.
...
5. **Gate Out (MANDATORY)** — Run `coder memory store "Planning Update: <Feature Name>" "<Summary of Progress, Risks, and Scope Changes>"` to update semantic memory.
