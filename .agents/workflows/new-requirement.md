...
4. **Requirements Phase** — Fill out `docs/ai/requirements/feature-{name}.md`: problem statement, goals/non-goals, user stories, success criteria, constraints, open questions. Apply any relevant best practices from Gate In skill results.
...
8. **Gate Out (MANDATORY)** — Use `coder memory store "Feature Design: <Name>" "<Key Architectural Decisions and Requirements Summary>"` to capture the state of documentation into memory.
