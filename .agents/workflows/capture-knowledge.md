...
1. **Gate In (MANDATORY)** — Run `coder skill search "<entry point or topic>"` to retrieve documentation patterns and analysis methodologies, then run `coder memory search "<entry point or topic>"` to retrieve historical context, patterns, and related knowledge before starting.
2. **Gather & Validate Entry Point** — If not already provided, ask for: entry point (file, folder, function, API), why it matters (feature, bug, investigation), and desired depth or focus areas. Confirm the entry point exists; if ambiguous or not found, clarify or suggest alternatives.
